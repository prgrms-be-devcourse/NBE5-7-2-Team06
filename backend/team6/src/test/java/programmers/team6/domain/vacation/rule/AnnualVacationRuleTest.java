...
import programmers.team6.domain.vacation.rule.builder.TestMemberVacationInfoBuilder;
...
class AnnualVacationRuleTest {
	...
	void 연차대상자_확인(Integer year, boolean expected) {
		...
		boolean result = annualVacationRule.isAnnualVacationEligible(date.plusYears(year).toLocalDate(),
			vacationGrantEligibility);

		assertThat(result).isEqualTo(expected);
	}

	@ParameterizedTest
	@CsvSource(value = {"2023-10-18T00:00:00,15", "2021-10-18T00:00:00,16", "2003-10-18T00:00:00,25",
		"2001-10-18T00:00:00,25"}, delimiter = ',')
	void 지급정보생성(LocalDateTime joinDate, int increaseCount) {
		...
		assertThat(grantInfo.totalCount()).isEqualTo(
			TestMemberVacationInfoBuilder.DEFAULT_VACATION_COUNT + increaseCount);
	}
